...
export default defineEventHandler(async (event) => {
    ...
    const existingTag = (article.tags ?? []).find((t: {tag_id: ObjectId}) => new ObjectId(t.tag_id).toString() === new ObjectId(tag_id).toString());
    ...
    if (existingTag) {
        ...
        if (existingTag.significance !== significance) {
            ...
            return {statusCode: 200, body: {message: 'Tag significance is already up to date'}};
        }
    } else {
        ...
        try {
            ...
            console.error(err);
            return {statusCode: 500, body: {error: 'Failed to add tag to article'}};
        }
    }
})
